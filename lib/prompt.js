...
prompt.delimiter              = ': ';
prompt.colors                 = true;
prompt.isDefaultValueEditable = false;
...
prompt.getInput = function (prop, callback) {
  ...
  raw = prompt.colors
    ? [prompt.message, colors.grey(name), colors.grey(delim)]
    : [prompt.message, name, delim];
  ...
  read({
    ...
    default: defaultLine().toString(),
    input: stdin,
    output: stdout,
    edit: prompt.isDefaultValueEditable
  }, function (err, line) {
    if (err && wait === false) {
      return callback(err);
    }
    ...
    // the callback.
    //
    logger.input(colors.yellow(line));
